...
program
	...
	.action(
		async (project: string | undefined, env: string | undefined, _opts: unknown, cmd: Command) => {
...
program
	...
	.action(async () => {
		...
		if (existing) {
			p.log.warn(
				`A ${chalk.bold('vaultsy.json')} already exists at ${chalk.dim(existing.dir)}.\n` +
		...
		const selectedEnv = await p.select({
			...
			initialValue: 'development' as Environment
		});
		...
		p.outro(
			`${chalk.green('✓')} Created ${chalk.bold('vaultsy.json')}\n` +
