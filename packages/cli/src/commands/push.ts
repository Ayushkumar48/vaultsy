...
import { readEnvFile, findProjectConfig, envFileName, type SecretRow } from '../lib/env.js';

export async function pushCommand(
	projectArg: string | undefined,
	envArg: string | undefined,
	opts: { input?: string; yes?: boolean }
): Promise<void> {
	...
	if (!existsSync(inputPath)) {
		p.log.error(
			`File ${chalk.bold(filename)} not found.\n` +
	...
	p.log.info(
		`Read ${chalk.bold(String(localSecrets.length))} secret${localSecrets.length !== 1 ? 's' : ''} ` +
...
function computeDiff(remote: SecretRow[], local: SecretRow[]): DiffResult {
...
function printDiff(diff: DiffResult): void {
	const total =
		diff.added.length + diff.modified.length + diff.removed.length + diff.unchanged.length;
