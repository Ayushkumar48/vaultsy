...
export async function pullCommand(
	...
	if (!opts.yes && !isGitIgnored(filename)) {
		p.log.warn(
			`${chalk.yellow(filename)} does not appear to be in ${chalk.dim('.gitignore')}.\n` +
	...
	let result: Awaited<ReturnType<typeof pullSecrets>>;
	try {
		result = await pullSecrets(projectId, env);
		spinner.stop(
			`Pulled ${result.secrets.length} secret${result.secrets.length !== 1 ? 's' : ''} ` +
	...
	p.outro(
		`${chalk.green('✓')} Written to ${chalk.bold(filename)}\n` + `  ${chalk.dim(outputPath)}`
	);
}
