...
export async function runCommand(
	...
	child.on('error', (err) => {
		if ((err as NodeJS.ErrnoException).code === 'ENOENT') {
			p.log.error(
				`Command not found: ${chalk.bold(bin)}\n` +
...
function secretsToRecord(secrets: { key: string; value: string }[]): Record<string, string> {
 ...
 */
function filterStringRecord(env: Record<string, string | undefined>): Record<string, string> {
