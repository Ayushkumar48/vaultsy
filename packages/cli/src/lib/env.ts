 ...
 */
export function serializeEnvText(rows: SecretRow[]): string {
 ...
 */
export function readEnvFile(filePath: string): SecretRow[] {
 ...
 */
export function writeEnvFile(filePath: string, rows: SecretRow[]): void {
...
): { config: VaultsyProjectConfig; dir: string } | null {
	...
	while (true) {
		...
		if (existsSync(candidate)) {
			try {
				...
				const parsed = JSON.parse(raw) as Partial<VaultsyProjectConfig>;
				if (parsed.project && typeof parsed.project === 'string') {
					return { config: parsed as VaultsyProjectConfig, dir: current };
				}
			} catch {
 ...
 */
export function writeProjectConfig(config: VaultsyProjectConfig, dir: string = process.cwd()): void {
 ...
 */
export function isGitIgnored(filename: string, dir: string = process.cwd()): boolean {
