...
export async function loginCommand(opts) {
    ...
    if (opts.token) {
        ...
        p.log.info(`Create a token at ${chalk.cyan(resolvedBaseUrl.replace(/\/$/, '') + '/dashboard/settings')}`);
    ...
    try {
        ...
        if (err instanceof ApiError) {
            ...
            p.log.error(`Could not reach ${resolvedBaseUrl}. Check the URL and your network connection.`);
        }
        process.exit(1);
    }
    // ── Persist config ───────────────────────────────────────────────────────
    ...
    p.outro(`${chalk.green('✓')} Logged in as ${chalk.bold(userName)} ${chalk.dim(`<${userEmail}>`)}\n` +
