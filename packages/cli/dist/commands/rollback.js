...
export async function rollbackCommand(projectArg, envArg, versionIdArg, opts) {
    ...
    if (versionIdArg) {
        ...
        try {
            ...
            spinner.stop(`Found ${versionsResult.versions.length} snapshot${versionsResult.versions.length !== 1 ? 's' : ''}.`);
        }
        catch (err) {
    ...
    // ── Confirm ──────────────────────────────────────────────────────────────
    if (!opts.yes) {
        const label = versionNumber !== undefined
            ? `v${versionNumber} (${chalk.dim(versionId)})`
            : chalk.dim(versionId);
        p.log.warn(`This will overwrite all ${chalk.bold(env)} secrets with the state from snapshot ${label}.\n` +
    ...
    try {
        ...
        spinner.stop(`Rolled back to v${result.rolledBackTo.versionNumber}. ` +
            `${chalk.green(`+${added}`)} added, ` +
            `${chalk.yellow(`~${modified}`)} modified, ` +
            `${chalk.red(`-${removed}`)} removed, ` +
            `${chalk.dim(`${unchanged} unchanged`)}.`);
        p.outro(`${chalk.green('✓')} ${chalk.bold(projectTitle ?? result.project.title)} / ${chalk.cyan(env)} ` +
