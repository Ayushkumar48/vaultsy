...
export async function historyCommand(projectArg, envArg) {
    ...
    try {
        ...
        spinner.stop(`${result.versions.length} snapshot${result.versions.length !== 1 ? 's' : ''} ` +
    ...
    const COL_BY = 20;
    const COL_DATE = 22;
    const header = chalk.bold(padEnd('#', COL_VER)) +
        chalk.dim(' │ ') +
    ...
    const divider = chalk.dim('─'.repeat(COL_VER) +
        '─┼─' +
        '─'.repeat(26) +
        '─┼─' +
        '─'.repeat(COL_SECRETS) +
    ...
    p.log.info(`To rollback, run: ${chalk.cyan(`vaultsy rollback ${projectId} ${env} <VERSION_ID>`)}`);
